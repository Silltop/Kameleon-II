...
def gather_facts():
    ...
    distro = execute_command("cat /etc/*-release 2>/dev/null | grep -v '/etc/upstream-release/' | awk -F '=' "
                             "'/^PRETTY_NAME/{print $2}' | tr -d '\"' ").strip('\n')
    users = execute_command("awk -F: '$6 ~ /^\/home/ { count++ } END { if (count > 0) print count; else print \"0\" "
                            "}' /etc/passwd").strip('\n')
    to_return = {"hostname": hostname, "kernel": kernel, "distro": distro, "users": users}
