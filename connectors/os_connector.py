...
class HostOutput:
    ...
    def merge_output_data(self) -> str:
        """Return data in one string, merge stdout, stderr and exception"""
        return_str = ""
        if self.stdout is not None:
            return_str += ''.join(self.stdout)
        if self.stderr is not None:
            return_str += ''.join(self.stderr)
        return_str += str(self.exception_details)
        return return_str

    def get_ip_with_output(self) -> Dict[str, str]:
...
class HostOutputManager:
    ...
    def get_output_per_host(self):
        return {k: v for entry in self.host_output_list for k, v in entry.get_ip_with_output().items()}


def execute_command_v2(command, hosts: tuple = None) -> HostOutputManager:
    ...
    for entry in command_output:
        ...
        if entry.exception is not None:
            ...
            if type(entry.exception) == ConnectionError:
